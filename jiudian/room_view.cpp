...
void RoomView::show_all()
{
	...
	for (auto& room : rooms)
	{
		cout << room.second.get_num() << "\t" << setw(10) << room.second.get_name() << "\t" << room.second.get_data() << "平方\t" << room.second.get_price() << "元\t" << room.second.get_bed_num() << "个\t\t" << room.second.show_state() << endl;
	}
	cout << "------------------------------------------------------------------------------------" << endl;
	//启动酒店管理系统
	void RoomView::run()
